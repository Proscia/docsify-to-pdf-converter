...
const combineMarkdowns = ({ contents, pathToDocsifyEntryPoint, pathToStatic, mainMdFilename }) => async links => {
  try {
    ...
    try {
      const content = files
        ...
        .map(processImagesPaths({ pathToDocsifyEntryPoint, pathToStatic }))
        .join("\n\n\n\n");
